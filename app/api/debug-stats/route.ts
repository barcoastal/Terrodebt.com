...
export async function GET() {
  ...
  try { result.recentLeads = await db.lead.findMany({ orderBy: { createdAt: "desc" }, take: 5, select: { id: true, firstName: true, lastName: true, businessName: true, source: true, createdAt: true, status: true, gclid: true, tkclid: true, affiliateClickid: true } }); } catch (e) { result.recentLeadsError = String(e); }
