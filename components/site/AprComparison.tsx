      ...
      <div className="mx-auto max-w-content px-6 py-20 md:py-24">
        <div className="max-w-3xl border-b border-rule pb-5">
          ...
          <p className="mt-4 text-base md:text-lg text-muted leading-relaxed">
            Factor rates obscure the real cost. Annualized, a typical aggressive merchant cash advance crosses 130 percent. APR is the right lens for MCA debt specifically. Other product categories use different math.
          </p>
        </div>

        <div className="mt-10 bg-cream border border-rule p-6 md:p-10">
          <div className="space-y-5">
            {ROWS.map((r) => (
              <div key={r.label}>
                ...
                <div className="mt-2 h-3 bg-rule rounded-full overflow-hidden">
                  <div className={`h-full ${r.color} rounded-full`} style={{ width: `${(r.value / max) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
          <div className="mt-8 pt-6 border-t border-rule flex flex-wrap items-center justify-between gap-3">
